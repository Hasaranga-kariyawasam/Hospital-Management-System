    ...
    if ($_POST['action'] === 'update_status') {
        ...
        try {
            ...
            if ($newStatus === 'resolved') {
                $pdo->prepare("
                    UPDATE emergency_requests SET resolved_at = NOW()
                    WHERE emergency_id = ?
                ")->execute([$reqId]);
...
if (isset($_GET['debug'])) {
    header('Content-Type: application/json');
    $aAll = $pdo->prepare("SELECT emergency_id, assigned_driver_id, status FROM emergency_requests ORDER BY emergency_id DESC LIMIT 5");
    $aAll->execute();
    echo json_encode([
        'user_id'               => $userId,
        'driver_id'             => $driver['driver_id'],
        'driver_status'         => $driver['status'],
        'assigned_emergency_id' => $driver['assigned_emergency_id'],
        'recent_emergencies'    => $aAll->fetchAll(),
    ], JSON_PRETTY_PRINT);
    exit;
}
            ...
            <?php foreach ($history as $h): ?>
                <tr>
                    ...
                    <td style="white-space:nowrap;color:#888;font-size:12px">
                        <?= !empty($h['completed_at']) ? date('d M Y, H:i', strtotime($h['completed_at'])) : '—' ?>
                    </td>
